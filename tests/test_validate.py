...
def test_duplicate_row_id_is_flagged():
    ...
    assert any("more than one Section 12 row" in w for w in warnings)
...
def test_orphan_row_never_referenced_is_flagged():
    ...
    warnings = validate_gap_ids(md)
    assert any("Section 12 row G-02 is never referenced" in w for w in warnings)
...
def _fork_sop(body: str) -> str:
    return f"## 7. Step-by-step procedure\n\n{body}\n## 11. End-state catalog\n"
...
def test_invented_node_is_flagged():
    ...
    assert any(
        "S9 has no matching Section 7 step (invented node)" in w for w in warnings
    )
