...
def test_validation_warnings_md_groups_both_kinds():
    section = _validation_warnings_md(
        ["Section 10 row G-03 is never referenced in the body."],
        [
            "End state 'Technical error' from Section 9 has no terminal node in the diagram."
        ],
    )
    assert "## Deterministic validation warnings" in section
    assert "### Gap-ID & fork-branch checks" in section
    assert "- Section 10 row G-03 is never referenced in the body." in section
